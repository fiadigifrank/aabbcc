...
install_XrayR() {
    ...
    if  [ $# == 0 ] ;then
        last_version=$(curl -Ls "https://api.github.com/repos/akiiya/XrayR/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ ! -n "$last_version" ]]; then
            echo -e "${red}检测 XrayR 版本失败，可能是超出 Github API 限制，请稍后再试，或手动指定 XrayR 版本安装${plain}"
            exit 1
        fi
        echo -e "检测到 XrayR 最新版本：${last_version}，开始安装"
        wget -N --no-check-certificate -O /usr/local/XrayR/XrayR-linux.zip https://github.com/akiiya/XrayR/releases/download/${last_version}/XrayR-linux-${arch}.zip
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/akiiya/XrayR/releases/download/${last_version}/XrayR-linux-${arch}.zip"
    ...
    file="https://github.com/akiiya/XrayR-script/raw/master/XrayR.service"
    ...
    curl -o /usr/bin/XrayR -Ls https://raw.githubusercontent.com/akiiya/XrayR-script/master/XrayR.sh
...
ConnetionConfig:
  ...
  DownlinkOnly: 30 # Time limit when the connection is closed after the uplink is closed, Second
...
stream {
    server {
        listen              443 ssl; listen 22053 ssl;      # 设置监听端口为443&22053
