...
install_base() {
    #load_module=""
    if [[ x"${release}" == x"centos" ]]; then
        load_module="load_module /usr/lib64/nginx/modules/ngx_stream_module.so"
...
#---------------------Get configuration
read -e -p 'Enter node domain:' domain
read -e -p 'Enter CA  server:' key_server
...
wget -O /etc/nginx/cert/${domain}/privkey.pem https://${key_server}/${ApiKey}/privkey.pem
wget -O /etc/nginx/cert/${domain}/fullchain.pem https://${key_server}/${ApiKey}/fullchain.pem
#-----------------------------------------------------------set-nginx----------------------------------------------
...
stream {
    server {
        listen 443 ssl;  listen 2053 ssl; listen 2083 ssl; listen 2087 ssl; listen 2096 ssl; listen 8443 ssl;               

        ssl_protocols      TLSv1 TLSv1.1 TLSv1.2 TLSv1.3;      # 设置使用的SSL协议版本

        ssl_certificate /etc/nginx/cert/${domain}/fullchain.pem; # 证书地址
        ssl_certificate_key /etc/nginx/cert/${domain}/privkey.pem; # 秘钥地址
